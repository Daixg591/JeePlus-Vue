     ...
     <div class="bg-white top">
        <vxe-toolbar :refresh="{query: refreshList}" export print custom>
          <template #buttons>
            ...
            <el-button v-if="hasPermission('test:shop:goods:edit')" type="warning" size="small" icon="el-icon-edit-outline" @click="edit()" :disabled="$refs.goods && $refs.goods.getCheckboxRecords().length !== 1" plain>修改</el-button>
            <el-button v-if="hasPermission('test:shop:goods:del')" type="danger"   size="small" icon="el-icon-delete" @click="del()" :disabled="$refs.goods && $refs.goods.getCheckboxRecords().length === 0" plain>删除</el-button>
          </template>
        </vxe-toolbar>
    ...
    methods: {
      ...
      del (id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          this.goodsService.delete(ids).then(({data}) => {
