...
<script>
  ...
  export default {
    ...
    testContinentService: null,
    created () {
      this.testContinentService = new TestContinentService()
    },
    methods: {
      init (method, id) {
        this.method = method
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.testContinentService.queryById(this.inputForm.id).then(({data}) => {
      ...
      doSubmit () {
        this.$refs['inputForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            this.testContinentService.save(this.inputForm).then(({data}) => {
              this.visible = false
