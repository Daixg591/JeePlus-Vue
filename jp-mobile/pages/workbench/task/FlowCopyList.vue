						...
						<view @tap="toDetail(row)" class="content">
								<view class="text-bold text-grey">
									<view class="ellipsis-description">
										{{row.createBy.name}}发起的【{{row.procInsName}}】
									</view>
								</view>
						</view>
						<view @tap="toDetail(row)" class="action">
							<view class="text-grey text-xs" > {{row.createDate  | formatDate}}</view>							
						</view>	  
...
<script>
	...
	export default {
		...
		methods: {
			...
			upCallback(page) {
				//联网加载数据
				this.$http.get('/app/extension/flowCopy/list', {
					pageNo: page.num,
					pageSize: page.size,
					procInsName: this.curWord
			  ...
			  del (row) {
				  uni.showModal({
				      ...
				      success: (res)=>{
				          if (res.confirm) {
				              this.$http.delete('/app/extension/flowCopy/delete?ids='+row.id).then(({data}) => {
			...
			toDetail (row) {
				this.$http.get('/app/flowable/task/getTaskDef', {
				  ...
				  if (data.success) {
					let query = {readOnly: true, title: row.procInsName, formTitle: row.procInsName, ...pick(data.flow, 'formType', 'formUrl', 'procDefKey', 'taskDefKey', 'procInsId', 'procDefId', 'taskId', 'status', 'title', 'businessId')}
					uni.navigateTo({
