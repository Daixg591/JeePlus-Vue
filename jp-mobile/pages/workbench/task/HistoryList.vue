			...
			<view class="cu-list menu-avatar">
				<view class="cu-item" :class="modalName=='move-box-'+ index?'move-cur':''" v-for="(row, index) in list" :key="index"
				 @touchstart="ListTouchStart" @touchmove="ListTouchMove"  @touchend="ListTouchEnd" :data-target="'move-box-' + index">
						<view @tap="toDetail(row)" class="content">
					...
					<view class="move" >
						<view v-if="row.isBack" class="bg-orange" @tap="callback(row)">撤回</view>
						<view class="bg-blue" @tap="toDetail(row)">历史</view>
					</view>
				</view>
			</view>
		</mescroll-body>
...
<script>
	...
	export default {
		...
		methods: {
			...
			upCallback(page) {
				//联网加载数据
				this.$http.get('/app/flowable/task/historic/', {
					pageNo: page.num,
					pageSize: page.size,
					status: '1',
					title: this.curWord
			...
			toDetail (row) {
				this.$http.get('/app/flowable/task/getTaskDef', {
				  ...
				  if (data.success) {
					let query = {readOnly: true, taskId: row.executionId, title: `${row.name}【${row.name}】`, formTitle: `${row.name}`, ...pick(data.flow, 'formType', 'formUrl', 'procDefKey', 'taskDefKey', 'procInsId', 'procDefId', 'taskId', 'status', 'title', 'businessId')}
					uni.navigateTo({
			 ...
			 // 取回
			  callback (row) {
				  ...
				  uni.showModal({
				      ...
				      success: (res) => {
				          if (res.confirm) {
				              this.$http.post('/app/flowable/task/callback',
