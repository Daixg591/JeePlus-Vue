				...
				<view class="cu-item" :class="modalName=='move-box-'+ index?'move-cur':''" v-for="(obj, index) in list" :key="index"
				 @touchstart="ListTouchStart" @touchmove="ListTouchMove"  @touchend="ListTouchEnd" :data-target="'move-box-' + index">
						...
						<view @click="toDetail(obj)" class="action">
							...
							<view :class="obj.readstatus === '1'?'bg-green':'bg-red'" class="cu-tag round  sm">
								{{obj.readstatus === '1'?'已读':'未读'}}
							</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			upCallback(page) {
				//联网加载数据
				this.$http.get('/app/mailBox/list', {
					pageNo: page.num,
					pageSize: page.size,
					mail: {
						title: this.curWord
			...
			del (id) {
				this.$http.delete('/app/mailBox/delete?ids='+id).then(({data})=>{
					uni.showToast({
						title: data.msg,
						icon:"success"
					})
