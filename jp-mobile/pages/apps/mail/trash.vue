				...
				<view class="cu-item" :class="modalName=='move-box-'+ index?'move-cur':''" v-for="(obj, index) in list" :key="index"
				 @touchstart="ListTouchStart" @touchmove="ListTouchMove"  @touchend="ListTouchEnd" :data-target="'move-box-' + index">
						...
						<view @click="toDetail(obj)" class="action">
							...
							<view v-if="obj.status === '0' " class="cu-tag bg-red round  sm">
								草稿
							</view>
							<view v-if="obj.status === '1' " class="cu-tag bg-green round  sm">
								已发送
							</view>
							<view v-if="obj.status === '2' " class="cu-tag bg-red round  sm">
								未读
							</view>
							<view v-if="obj.status === '3' " class="cu-tag bg-green round  sm">
								已读
							</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			upCallback(page) {
				//联网加载数据
				this.$http.get('/app/mail/mailTrash/list', {
					pageNo: page.num,
					pageSize: page.size,
					status: '1',
					mail: {
			...
			del (id) {
				this.$http.delete('/app/mail/mailTrash/delete?ids='+id).then(({data})=>{
					uni.showToast({
						title: data.msg,
						icon:"success"
					})
